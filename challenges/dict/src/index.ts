...
export function mapDict<T, S>(dictionary: Dict<T>, cb: (arg: T) => S): Dict<S> {
  ...
  Object.keys(dictionary).forEach(k => mappedDictionary[k] = cb(dictionary[k]));
  return mappedDictionary
}

// Array.prototype.reduce, but for Dict
export function reduceDict<T, S>(dictionary: Dict<T>, reducer: (arg1: S, arg2: T) => T, initValue = undefined): S {
